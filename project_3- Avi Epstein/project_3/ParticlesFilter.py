...
class ParticlesFilter:
    ...
    def motionModel(self, odometry):
      ...
      for i in range(self.numberOfParticles):
        ...
        dr1 = odometry['r1'] + np.random.normal(0,self.sigma_r1)#+ np.random.normal(0,np.power(self.sigma_r1,2))
        dt = odometry['t'] + np.random.normal(0,self.sigma_t) #+ np.random.normal(0,np.power(self.sigma_t,2))
        dr2 = odometry['r2'] + np.random.normal(0,self.sigma_r2)
    ...
    def Observation(self):
      ...
      for i in range(self.numberOfParticles):
        ...
        r_partical = np.sqrt(np.power(closeset_landmark[0] - self.particles[i][0],2) + np.power(closeset_landmark[1] - self.particles[i][1],2)) + np.random.normal(0,np.sqrt(self.Q[0][0])) #possibly add noise ask roy
        phi_partical = np.arctan2(closeset_landmark[1] - self.particles[i][1],closeset_landmark[0] - self.particles[i][0])+ np.random.normal(0,np.sqrt(self.Q[1][1])) #possibly add noise ask roy
    ...
    def resampleParticles(self):
      ...
      for i in range(0,self.numberOfParticles):
        ...
        if self.particles[i][3]< 1/(100*self.numberOfParticles):
